...
document.querySelectorAll("[data-countdown]").forEach(function (elem) {
  const countDownItem = function (value, label) {
    return `
        ...
        <span class="countdown__number">${toBangla(value).padStart(2, '০')}</span>
  ...
  const countDownInterval = setInterval(function () {
    ...
    const timeDistance = date - currentTime;
    const daysValue = Math.floor(timeDistance / day);
    const hoursValue = Math.floor((timeDistance % day) / hour);
    const minutesValue = Math.floor((timeDistance % hour) / minute);
    const secondsValue = Math.floor((timeDistance % minute) / second);
