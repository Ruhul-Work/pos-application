...
@section('content')
  <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
    <div>
      <h6 class="fw-semibold mb-0">Expense Category List</h6>
      <p class="m-0" >Manage Expense Categories</p>
    </div>
    <ul class="d-flex align-items-center gap-2">
      ...
      <li>-</li>
      <li class="fw-medium">Expense Categories</li>
    </ul>
  </div>
  ...
  <div class="card basic-data-table">
    <div class="card-header d-flex align-items-center justify-content-between">
      ...
      <div class="actions-bar d-flex align-items-center gap-2 flex-wrap">
        ...
        @perm('expenseCategories.store')
            <button 
                class="d-flex btn btn-primary btn-sm px-12 py-8 radius-8 AjaxModal"
                        data-ajax-modal="{{ route('expenseCategories.create') }}"
                        data-size="lg"
                        data-onload="CategoryIndex.onLoad"
                        data-onsuccess="CategoryIndex.onSaved">
                <iconify-icon icon="ic:baseline-plus" class="text-xl"></iconify-icon>Add Expense Category
        </button>
        @endperm
      </div>
    </div>
...
@section('script')
  <script>
    var DATATABLE_URL = "{{ route('expenseCategories.list.ajax') }}";
