...
@section('content')
  <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
    <div>
      <h6 class="fw-semibold mb-0">Product-type List</h6>
      <p class="m-0" >Manage Product-types</p>
    </div>
    <ul class="d-flex align-items-center gap-2">
      ...
      <li>-</li>
      <li class="fw-medium">Product-types</li>
    </ul>
  </div>
      ...
      <div class="actions-bar d-flex align-items-center gap-2 flex-wrap">
        ...
        @perm('org.branch.store')
            <button 
                ...
                data-ajax-modal="{{ route('product-type.product-types.createModal') }}"  
                data-size="lg"
                data-onsuccess="BranchesIndex.onSaved">
                <iconify-icon icon="ic:baseline-plus" class="text-xl"></iconify-icon>Add Product-type
        </button>
        @endperm
      </div>
    </div>
...
@section('script')
  <script>
    var DATATABLE_URL = "{{ route('product-type.product-types.list.ajax') }}";
  ...
  $(document).on('click', '.btn-branch-delete', function(e){
    ...
    const doDelete = () => {
      $.ajax({
        ...
        error: function(xhr){
          if (xhr.status === 422){
            const msg = xhr.responseJSON?.msg || 'Cannot delete this Product-type.';
    ...
    if (window.Swal){
      ...
      if (confirm('Delete this Product-type?')) doDelete();
    }
  });
