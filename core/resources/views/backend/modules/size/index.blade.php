...
@section('content')
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
        <div>
            <h6 class="fw-semibold mb-0">Size List</h6>
            <p class="text-muted m-0">All Sizes</p>
        </div>
        ...
        <div class="card-header d-flex align-items-center justify-content-between">
            ...
            <div class="actions-bar d-flex align-items-center gap-2 flex-wrap">
                ...
                <button class="d-flex btn btn-primary btn-sm px-12 py-8 radius-8 AjaxModal"
                    data-ajax-modal="{{ route('sizes.createModal') }}" data-size="md" data-onsuccess="SizesIndex.onSaved">
                    <iconify-icon icon="ic:baseline-plus" class="text-xl"></iconify-icon> Add Size
                </button>
            </div>
        </div>

        <div class="card-body">
            <table class="table bordered-table table-scroll mb-0 AjaxDataTable" id="sizesTable" style="width:100%">
                <thead>
                    <tr>
                        <th style="width:60px">
...
@section('script')
    <script>
        var DATATABLE_URL = "{{ route('sizes.list.ajax') }}";
        ...
        // Delete
        $(document).on('click', '.btn-size-delete', function(e) {
            e.preventDefault();
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        ...
                        if (xhr.status === 422) {
                            icon = 'warning';
                            title = 'Blocked';
                            text = xhr.responseJSON?.msg || 'Cannot delete this size.';
            ...
            } else if (confirm('Delete this size?')) doDelete();
        });
    </script>
@endsection
