<div class="modal-header py-16 px-24 border-0" data-modal-key="branch-create">
  <h5 class="modal-title">Edit Paper-quality</h5>
  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
</div>

<div class="modal-body p-24">
  <form id="branchCreateForm" action="{{ route('paper_quality.update', $paperQuality->id) }}" method="post" data-ajax="true" >
        @method('PUT') 
    @csrf
    <div class="row">
      <div class="col-md-6 mb-20">
        <label class="form-label text-sm mb-8">Name <span class="text-danger">*</span></label>
        <input type="text" name="name" class="form-control radius-8" placeholder="e.g. Cloth" value="{{$paperQuality->name}}" required>
      ...
      <div class="col-md-6 mb-20">
        ...
        <input type="text" name="code" class="form-control radius-8" placeholder=""  value="{{$paperQuality->code}}" required >
...
{{-- active toggle button --}}
      <div class="col-12 mb-8">
        ...
        <div class="form-switch switch-purple d-flex align-items-center gap-3">
          ...
          <input class="form-check-input" type="checkbox" name="is_active" value="1" id="branchIsActive" {{($paperQuality->is_active?'checked':'')}}>
          <label class="form-check-label" for="branchIsActive">Enable this Paper-quality</label>
        </div>
