...
@section('content')
  <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
    <div>
      <h6 class="fw-semibold mb-0">District List</h6>
      <p class=" m-0">Manage Districts</p>
    </div>
  ...
  <div class="card basic-data-table">
    <div class="card-header d-flex align-items-center justify-content-between">
      ...
      <div class="actions-bar d-flex align-items-center gap-2 flex-wrap">
        ...
        @perm('district.districts.store')
            <button 
                class="d-flex btn btn-primary btn-sm px-12 py-8 radius-8 AjaxModal"
                data-ajax-modal="{{ route('district.districts.createModal') }}"  
                data-size="lg"
                data-onsuccess="BranchesIndex.onSaved">
                <iconify-icon icon="ic:baseline-plus" class="text-xl"></iconify-icon>Add District
        </button>
        @endperm
      </div>
    </div>
...
@section('script')
  <script>
    var DATATABLE_URL = "{{ route('district.districts.list.ajax') }}";
  ...
  $(document).on('click', '.btn-branch-delete', function(e){
    ...
    if (window.Swal){
      ...
      if (confirm('Delete this district?')) doDelete();
    }
  });
