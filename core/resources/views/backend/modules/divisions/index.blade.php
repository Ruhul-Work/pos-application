...
@section('content')
  <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
    <div>
      <h6 class="fw-semibold mb-0">Division List</h6>
    ...
    <ul class="d-flex align-items-center gap-2">
      ...
      <li>-</li>
      <li class="fw-medium">Divisions</li>
    </ul>
  </div>
  ...
  <div class="card basic-data-table">
    <div class="card-header d-flex align-items-center justify-content-between">
      ...
      <div class="actions-bar d-flex align-items-center gap-2 flex-wrap">
        ...
        @perm('division.divisions.store')
            <button 
                class="d-flex btn btn-primary btn-sm px-12 py-8 radius-8 AjaxModal"
                data-ajax-modal="{{ route('division.divisions.createModal') }}"  
                data-size="lg"
                data-onsuccess="BranchesIndex.onSaved">
                <iconify-icon icon="ic:baseline-plus" class="text-xl"></iconify-icon>Add Division
        </button>
        @endperm
      </div>
    </div>
...
@section('script')
  <script>
    var DATATABLE_URL = "{{ route('division.divisions.list.ajax') }}";
  ...
  $(document).on('click', '.btn-branch-delete', function(e){
    ...
    const doDelete = () => {
      $.ajax({
        ...
        error: function(xhr){
          if (xhr.status === 422){
            const msg = xhr.responseJSON?.msg || 'Cannot delete this division.';
