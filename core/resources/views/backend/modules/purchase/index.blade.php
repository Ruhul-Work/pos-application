...
$(function () {
    ...
    (function seedEditContext() {
        ...
        const serverItems = (window.EDIT_ORDER.items || []).map(i => ({
            ...
            stock_quantity: (i.product && i.product.stock_quantity) ? i.product.stock_quantity : undefined,
            sku: i.sku ?? null,
    ...
    function triggerSystemQtyRefresh() {
        ...
        // debug
        console.log('triggerSystemQtyRefresh -> fetching pids', pids, 'wid', getCurrentWarehouseId(), 'bid', getCurrentBranchId());
    ...
    function bulkFetchSystemQty(productIds) {
        ...
        console.log('bulkFetchSystemQty -> sending', { productIds, wid, bid });
            ...
            success(res) {
                ...
                productIds.forEach(pid => {
                    ...
                    if (Object.prototype.hasOwnProperty.call(map, pid)) {
                        const val = map[pid];
                        const sysVal = (val !== undefined && val !== null && !isNaN(parseFloat(val))) ? parseFloat(val).toFixed(2) : '0.00';
                        $row.find('.sys-qty').text(sysVal);
                        $row.find('.row-warehouse').val(wid);
                        $row.find('.row-branch').val(bid);
                    } else {
                        console.warn(`bulkFetchSystemQty: no value from server for pid ${pid} — preserving existing`);
                    }
                });
            },
...
<tr data-vid="${item.id}" data-product_id="${item.id}">
    ...
    function calculateSubtotal() {
        ...
        if (discountObj && Number(discountObj.value)) {
            if (discountObj.type === 'percentage') discountAmount = (subtotal * parseFloat(discountObj.value) / 100);
    ...
    $(document).on('input', '.qty-input', function() {
        ...
        const cart = getCart();
        const product = cart.find(i => i.id === productId);
        if (product) tr.find('.product-price').html((product.cost_price * product.quantity).toFixed(2));
        calculateSubtotal();
    });
    ...
    $(document).on('input', '.item_price', function() {
        ...
        if (product) tr.find('.product-price').html((product.quantity * product.cost_price).toFixed(2));
    });
    ...
    $warehouse.on('select2:select', function(evt) {
        ...
        $.getJSON(url)
            ...
            .fail(function(err) { console.warn('warehouse detail ajax failed', err); })
            .always(function() { triggerSystemQtyRefresh(); });
    });
    ...
    function postPurchase(payload, invoiceFile = null, successCb = null, errorCb = null) {
        ...
        $.ajax({
            ...
            processData: false,
            contentType: false,
            success: function(res) { if (successCb) successCb(res); },
            error: function(xhr) { if (errorCb) errorCb(xhr); else console.error(xhr.responseText || xhr); }
        });
    }
    ...
    $(document).on('click', '#submit_purchase_btn, .submit-purchase', function(e) {
        ...
        postPurchase(payload, invoiceFile, function(res) {
            ...
            if (res.success) {
                ...
                emptyCart();
                if (res.redirect_url) setTimeout(() => window.location.href = res.redirect_url, 600);
    ...
    function loadSupplier() {
        ...
        $.get(url).done(function(res) {
            if (res.supplier) $('#supplier').append(new Option(res.supplier.name, res.supplier.id, true, true)).trigger('change');
        }).fail(function(xhr) { console.error(xhr.responseText); });
    }
    if ($('#supplier').length) loadSupplier();

    function shortName(name, letterLimit = 15) {
        if (!name) return '';
