    ...
    <div class="container">
        ...
        <div class="row mb-3">
            ...
            <div class="d-flex justify-content-between align-items-center">
                <button class="btn btn-sm btn-warning AjaxModal" id="btn-add-payment"
                    data-onsuccess="purchasePayment.onSaved" data-order-id="{{ $order->id }}"
                    data-ajax-modal="{{ route('purchase.orders.payment.modal', $order->id) }}">Add
                    Payment</button>

                <div >
                    @if ($order->status === 'draft')
                        <button id="btn-receive-all " class="btn btn-sm btn-success AjaxModal" data-ajax-modal="{{ route('purchase.orders.receive-all.modal', $order->id) }}" data-onsuccess="purchaseReceipt.onReceived">Receive All</button>
                    @else
