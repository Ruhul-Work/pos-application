...
@section('meta')
  <title>upazilas</title>
@endsection

@section('content')
  <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
    <div>
      <h6 class="fw-semibold mb-0">Upazila List</h6>
      <p class="m-0">Manage  upazilas</p>
    </div>
    <ul class="d-flex align-items-center gap-2">
      ...
      <li>-</li>
      <li class="fw-medium">upazilas</li>
    </ul>
  </div>
  ...
  <div class="card basic-data-table">
    <div class="card-header d-flex align-items-center justify-content-between">
      ...
      <div class="actions-bar d-flex align-items-center gap-2 flex-wrap">
        ...
        @perm('upazila.upazilas.store')
            <button 
                class="d-flex btn btn-primary btn-sm px-12 py-8 radius-8 AjaxModal"
                data-ajax-modal="{{ route('upazila.upazilas.createModal') }}"  
                data-size="lg"
                data-onsuccess="BranchesIndex.onSaved">
                <iconify-icon icon="ic:baseline-plus" class="text-xl"></iconify-icon>Add Upazila
        </button>
        @endperm
      </div>
    </div>
...
@section('script')
  <script>
    var DATATABLE_URL = "{{ route('upazila.upazilas.list.ajax') }}";
  ...
  $(document).on('click', '.btn-branch-delete', function(e){
    ...
    const doDelete = () => {
      $.ajax({
        ...
        error: function(xhr){
          if (xhr.status === 422){
            const msg = xhr.responseJSON?.msg || 'Cannot delete this Upazila.';
    ...
    if (window.Swal){
      ...
      if (confirm('Delete this upazila?')) doDelete();
    }
  });
