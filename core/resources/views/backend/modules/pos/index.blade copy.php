                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="row">
                                            <div class="mb-3 col-md-6">
                                                <label for="received_amount" class=" col-form-label">Sale Status</label>
                                                <select name="sale_status" id="sale_status"
                                                    class="form-control form-control-sm">
                                                    <option value="deliverd" selected>Delivered</option>
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-body text-dark p-0 px-3">
                            <img src="http://localhost/pos-application/theme/admin/assets/images/logo1.png"
                                class="mt-0" alt="img" style="height: 32px" >
...
@section('script')
    <script>
        $(document).ready(function() {
            ...
            // ====== tab-specific cart helpers ======
(function() {
    // ensure tabId exists in sessionStorage
    function getOrCreateTabId() {
        let tabId = sessionStorage.getItem('pos_tab_id');
        if (!tabId) {
            // simple random id (you can use better UUID lib if you want)
            tabId = 'tab_' + Date.now() + '_' + Math.floor(Math.random() * 1000000);
            sessionStorage.setItem('pos_tab_id', tabId);
        }
        return tabId;
    }

    // return storage key used for this tab
    function getCartKey() {
        return 'cart_' + getOrCreateTabId();
    }

    // read cart for THIS tab
    ...
    // write cart for THIS tab
    window.setCart = function(cart) {
        localStorage.setItem(getCartKey(), JSON.stringify(cart || []));
        // emit custom event (optional) if you want other parts of same tab listen
        window.dispatchEvent(new CustomEvent('pos:cart:updated', { detail: { tabKey: getCartKey(), cart } }));
    };

    // remove cart for THIS tab
    window.clearCartForTab = function() {
        localStorage.removeItem(getCartKey());
        window.dispatchEvent(new CustomEvent('pos:cart:cleared', { detail: { tabKey: getCartKey() } }));
    };

    // get list of all open POS tab keys (optional helper)
    window.getAllCartKeys = function() {
        return Object.keys(localStorage).filter(k => k.startsWith('cart_tab_') || k.startsWith('cart_tab'));
        // note: our key is 'cart_tab...' but above uses 'cart_<tabId>' - if you want change detection adapt accordingly
    };

    // Expose tab id
    window.getPosTabId = getOrCreateTabId;
})();
