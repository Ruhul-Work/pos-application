...
@section('meta')
    <title>{{ $author->name ?? 'Author Products' }} | {{ get_option('title') }}</title>
    <meta property="og:title" content="{{ $author->name }} | {{ strtolower($author->meta_title ?? get_option('title')) }}">
    <meta property="og:description" content="{{ strip_tags($author->meta_description ?? get_option('description')) }}">
    <meta property="og:type" content="website">
    <meta property="og:url" content="{{ url()->current() }}">
    <meta property="og:image" content="{{ asset($author->meta_image ?? get_option('meta_image')) }}">
    <meta property="og:site_name" content="{{ get_option('company_name') }}">
    <!-- Add more Open Graph tags as needed -->
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:title" content="{{ $author->name }} | {{ strtolower($author->meta_title ?? get_option('title')) }}">
    <meta name="twitter:description" content="{{ strip_tags($author->meta_description ?? get_option('description')) }}">
    <meta name="twitter:image" content="{{ asset($author->meta_image ?? get_option('meta_image')) }}">
...
@section('content')
    ...
    @if(!empty($author->icon))
        ...
        <section class="author-bg-color">
            <div class="container ">


                <div class="row align-items-center publication-image">
                    <div class="col-md-12 text-center">
                        <img class="shadow" src="{{asset($author->icon)}}" alt="{{$author->name}}">
                    </div>

                    <div class=" publication-title col-md-12 text-center py-4">
                        <h2>{{$author->name}}</h2>
                        <p>
                            {{$author->description}}
                        </p>
            ...
            <div class="row row-cols-1">
                <div class="col">
                    <div class="breadcrumb__content text-center">
                        <h1 class="breadcrumb__content--title text-dark mb-25">{{$author->name}}</h1>
                        <ul class="breadcrumb__content--menu d-flex justify-content-center">
                            <li class="breadcrumb__content--menu__items"><a class="text-dark" href="{{route('home')}}">হোম</a></li>
                            <li class="breadcrumb__content--menu__items"><a class="text-dark" href="{{route('author.all')}}">লেখক বৃন্দ</a></li>
                            <li class="breadcrumb__content--menu__items"><span class="text-dark">{{$author->name}}</span></li>
                        </ul>
                    </div>
                </div>
            </div>
    ...
    <section class="shop__section section--padding">
        <div class="section__heading text-center mb-50">
                    <h2 class="section__heading--maintitle">লেখকের বইসমূহ</h2>
                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function fetchProducts(filters = {}, sortBy = 'latest', page = 1) {
                ...
                $.ajax({
                    url: "{{ route('author.products', ['slug' => $author->slug]) }}",
                    type: 'GET',
                    data: filters,
            ...
            function applyInitialFilters() {
                ...
                if (filters.categoryIds || filters.publicationIds) {
