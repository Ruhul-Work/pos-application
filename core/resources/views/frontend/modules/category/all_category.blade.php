            ...
            <div class="row row-cols-1">
                <div class="col">
                    <div class="breadcrumb__content text-center">
                        <h1 class="breadcrumb__content--title mb-25">বিষয়</h1>
                        <ul class="breadcrumb__content--menu d-flex justify-content-center">
                            <li class="breadcrumb__content--menu__items">
                                <a class="text-dark" href="{{ route('home') }}">হোম</a>
                            </li>
                            <li class="breadcrumb__content--menu__items">
                                <span class="">বিষয়</span>
                            </li>
                        </ul>
                    </div>
                </div>
            ...
            <div class="row mb-30">
                <div class="col-md-6 mx-auto">
                    ...
                    <div class="product__view--mode__list product__view--search d-flex justify-content-center">
                        <form class="product__view--search__form" id="category-search-form">
                            <label>
                                <input class="product__view--search__input border-1" placeholder="বিষয় অনুসন্ধান করুন" type="text" id="category-search">
                            </label>
    ...
    <script>
        $(document).ready(function() {
            ...
            function performSearch() {
                var query = $('#category-search').val().trim();
                if (query.length === 0) {
                    ...
                    success: function(response) {
                        ...
                        if (response.categories.length > 0) {
                            response.categories.forEach(function(category) {
                                let categoryUrl = "{{ route('category.single', ['slug' => ':slug']) }}".replace(':slug', category.slug);
                                $('#search-result').append(
                                    `<li><a href="${categoryUrl}">${category.name}</a></li>`
                                );
                            });
                        } else {
                            $('#search-result').append('<li>No categories found</li>');
                        }
