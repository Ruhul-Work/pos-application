...
@section('meta')
    ...
    @if ($search)
        <title>Products By {{ ucwords($search) }} | {{ get_option('title') }}</title>
    @else
        <title>Products By Keyword Search | {{ get_option('title') }}</title>
    @endif

    @if(!empty($search))
        <meta property="og:title" content="{{ ucwords($search) }}">
        <meta property="og:type" content="website">
        <meta property="og:description" content="All kinds of books related to {{ ucwords($search) }} available at reasonable prices with the best quality.">
        ...
        <!-- Twitter Card data -->
        <meta name="twitter:title" content="{{ ucwords($search) }}">
        <meta name="twitter:card" content="summary_large_image">
        <meta name="twitter:url" content="{{ url()->current() }}">
        <meta name="twitter:image" content="{{ asset(get_option('meta_image')) }}">
        <meta name="twitter:description" content="All kinds of books related to {{ ucwords($search) }} available at reasonable prices with the best quality.">
            ...
            <div class="row row-cols-1">
                <div class="col">
                    <div class="breadcrumb__content text-center">
                        <h1 class="breadcrumb__content--title text-dark mb-25">{{$search}}</h1>
                        <ul class="breadcrumb__content--menu d-flex justify-content-center">
                            ...
                            <li class="breadcrumb__content--menu__items"><span
                                    class="text-dark">{{$search}}</span></li>
                        </ul>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            function fetchProducts(filters = {}, sortBy = 'latest', page = 1) {
                ...
                $.ajax({
                    url: "{{ route('search.products', ['search' => $search, 'productType'=>$productType]) }}",
